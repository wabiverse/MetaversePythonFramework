...
class _Processor:
    ...
    # On xrOS, iOS, tvOS and watchOS, os.uname returns the
    # architecture as uname.machine. On device it doesn't;
    # but there's only one CPU architecture on device
    def get_xros():
...
def uname():
    ...
    if sys.platform in ('xros', 'ios', 'tvos'):
        ...
        # On xrOS/iOS/tvOS simulators, os.uname() reports
        # the machine as something like "arm64" or "x86_64".
        if getattr(sys.implementation, "_simulator", False):
